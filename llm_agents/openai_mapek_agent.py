...
class MapeK4OpenAIAgent(OpenAIAgent):
    ...
    async def answer_user_question(self, user_question: str):
        with trace("Deterministic MAPE-K flow"):
            ...
            mon_res = mon_res.final_output
            self.log_component_input_output("monitor", mon_input, mon_res)

            # Update User model
            if mon_res.mode_of_engagement:
                self.user_model.cognitive_state = self.modes_of_engagement.get_differentiating_description(
                    mon_res.mode_of_engagement)
            if mon_res.explicit_understanding_displays:
                self.user_model.explicit_understanding_signals = mon_res.explicit_understanding_displays
            ...
            anl_res = anl_res.final_output
            self.log_component_input_output("analyze", anl_input, anl_res)
            ...
            self.log_component_input_output("plan", plan_input, plan_res)

            # Update explanation plan
            if plan_res.explanation_plan:
                self.explanation_plan = plan_res.explanation_plan

            # 4) Execute
            exec_input = create_execute_user_prompt(
                chat_history=self.chat_history,
                user_message=user_question,
                next_exp_content=plan_res.next_response[
                    0].communication_goals if plan_res.next_response else "",
            )
            exec_res = await Runner.run(self.execute_agent, exec_input)
            exec_res = exec_res.final_output
            self.log_component_input_output("execute", exec_input, exec_res)
            ...
            return exec_res.reasoning, exec_res.response
...
class MapeK2OpenAIAgent(OpenAIAgent):
    ...
    async def answer_user_question(self, user_question: str):
        with trace("MAPE-K 2-step flow: Monitor+Analyze, then Plan+Execute"):
            ...
            self.log_component_input_output("monitor_analyze", ma_input, ma_res)
...
class MapeKUnifiedOpenAIAgent(OpenAIAgent):
    ...
    async def answer_user_question(self, user_question: str):
        with trace("Unified single-prompt MAPE-K flow"):
            ...
            self.log_component_input_output("unified_agent", unified_input, unified_res)

            # Update user model from unified result
            if unified_res.mode_of_engagement:
                self.user_model.cognitive_state = self.modes_of_engagement.get_differentiating_description(
                    unified_res.mode_of_engagement)
            if unified_res.explicit_understanding_displays:
                self.user_model.explicit_understanding_signals = unified_res.explicit_understanding_displays

            # Process model changes
            for change in getattr(unified_res, "model_changes", []):
                if isinstance(change, dict):
            ...
            # Update explanation plan and shown explanations
            if unified_res.explanation_plan:
                self.explanation_plan = unified_res.explanation_plan
            if hasattr(unified_res, "new_explanations") and unified_res.new_explanations:
                self.last_shown_explanations.extend(unified_res.new_explanations)
            ...
            row = {
                ...
                "datapoint_count": self.datapoint_count,
                "user_message": user_question,
                "monitor": {"mode_of_engagement": unified_res.mode_of_engagement,
                            "explicit_understanding_displays": unified_res.explicit_understanding_displays},
                "analyze": getattr(unified_res, "model_changes", []),
                "plan": {"new_explanations": getattr(unified_res, "new_explanations", []),
                         "explanation_plan": unified_res.explanation_plan},
                "execute": {"reasoning": unified_res.reasoning,
                            "response": unified_res.response},
                "user_model": self.user_model.get_state_summary(as_dict=False),
            }
            append_new_log_row(row, self.log_file)
            update_last_log_row(row, self.log_file)

            return unified_res.reasoning, unified_res.response
...
Respond naturally and helpfully to user questions about the AI prediction and explanations."""
    ...
    async def answer_user_question(self, user_question: str):
        ...
        with trace("Simple direct question answering"):
            ...
            self.log_component_input_output("simple_qa_agent", f"User: {user_question}", result.final_output)
