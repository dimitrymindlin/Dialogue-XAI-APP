...
def filter_by_wanted_count(analysis, count=70):
    # Filter participants by time (Take first count participants) per study group, sorted by prolific_start_time
    sorted_user_ids = analysis.user_df.sort_values("prolific_start_time")["id"]
    # First, 70 interactive users
    interactive_ids = sorted_user_ids[
        sorted_user_ids.isin(analysis.user_df[analysis.user_df["study_group"] == "interactive"]["id"])]
    interactive_ids = interactive_ids[:count]
    # Then, 70 static users
    static_ids = sorted_user_ids[
        sorted_user_ids.isin(analysis.user_df[analysis.user_df["study_group"] == "static"]["id"])]
    ...
    analysis.update_dfs(user_ids_to_remove)
